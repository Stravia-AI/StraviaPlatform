...
> 本文件是 Stravia WebUI 的设计规范，结构遵循 [Google Labs `design.md` 规范](https://github.com/google-labs-code/design.md/blob/main/docs/spec.md)。YAML token 保留当前视觉基础；正文规定目标交互与使用边界，不代表每条要求已实现。视觉实现复用 `frontend/stravia-webui/src/app.css` 和现有 UI 组件；交互改动按对应 spec 落地，不另建平行设计系统。
