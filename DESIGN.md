...
description: 面向个人开发者的本地 Agent infra 管理工作台，以轻量任务衔接、准确文案和常规操作反馈帮助用户接入模型与客户端、配置平台能力并观察执行。
colors:
...
Stravia 是本地运行、可自托管的 Agent infra（智能体基础设施），协议网关是其模型接入层。管理界面优先服务个人开发者：连接所需模型服务，为 Codex、Claude Code 等第三方工具配置模型访问，配置联网搜索、多模态理解等平台能力，并在需要时查看执行记录、用量与诊断。既有管理能力保留，但不以多人密钥分配或批量运维组织主流程，也不引入用户自定义 Agent 或可视化工作流编辑器。界面首先帮助用户回答：**现在该做什么、客户端可以用什么模型、操作是否完成、出问题后怎么继续**。
...
- 模型同步成功使用 Sonner toast 显示结果和新增、不再提供、恢复提供数量，不保留页面级成功通知块；模型清单仍保留上次检查摘要。
...
- 添加成功使用 Sonner toast 显示结果，不保留页面级成功通知块；可继续添加其他模型，不强制跳页。接入客户端沿用导航与概览入口。
...
- 联网搜索仅有一个能力总开关。Stravia 模型模式下的搜索、网页读取来源与优先级属于配置，不再设置第二个总开关；来源选择使用 Checkbox，与顶部启停 Switch 区分。缺少可用模型或来源时就地说明补齐动作，不用常驻“更改立即生效”解释内部保存机制。
...
- 请求记录的交互链路与失败请求列表工作区填满标题、提示和工具栏之后的窗口剩余高度，不使用独立 `vh` 限高或固定最小高度撑出页面滚动。失败请求列表在工作区内部滚动，末条记录与加载更多操作必须可达；全屏沿用同一空间分配方式。
...
Lucide 图标使用 16px 为常规尺寸，跟随文字颜色；图标只辅助识别，不能代替按钮名称或 aria-label。Provider mark 固定 28×28px：优先本地图标或安全来源图标；缺失时使用主色淡底与首字母，不出现纯黑占位块。自定义服务例外：选择入口及站点图标缺失时使用 Lucide `Plug`，不使用名称首字。
...
- `task brand:generate` 从唯一 SVG 源生成静态黑白版本、自适应 SVG favicon、透明底黑色应用 SVG 及 Tauri 所需的 PNG／ICO／ICNS；不得手工编辑导出文件。不得在 WebUI 中优先加载位图 favicon。
- 系统任务栏、原生窗口与托盘统一使用透明底黑色节律图形，不跟随应用或系统明暗主题切换；应用界面中的 `BrandMark` 继续继承 `foreground`。macOS 托盘仍使用系统 template image，以遵循菜单栏的原生对比度处理。
...
- 凭据保护的规则工作区使用窗口剩余高度，分页器保留正常底部内边距；标题、开关和工具栏按内容占位，不用固定表格限高在分页器下方留下大块空白。窄屏与低窗口允许页面滚动，保证规则区和分页操作可达。
...
- 模型编辑器的 Target 状态来自独立的运行态查询，不进入编辑草稿。正常使用绿色圆点，冷却中、等待重试和重试中使用黄色警示形状与文字；加载、未保存和读取失败不得伪装成正常。窄 Target 卡将状态换行，保留服务、模型与优先级的识别空间。
...
- Desktop 在网关就绪前使用不依赖业务存储或 HTTP 的启动界面；关键初始化失败时保留恢复窗口，提供可展开诊断、复制、可用日志入口、重启和退出，不跳转登录页。静默启动失败也必须可见；失败时关闭窗口退出，只有正常运行且托盘可用时才隐藏。非关键桌面集成失败在正常界面提示，不伪装成网关不可用。
