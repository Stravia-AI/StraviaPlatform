...
- 选路通过 `UsageStatsStore` 读取 `target_attempt_observations` 的 24h/1h Confirmed Upstream Usage 聚合，并叠加进行中占位；冷却保持进程内。存储查询失败时沿用进程内最后一次成功快照并标记 stale；尚无快照或 Observation gap 导致数据缺失时，以无历史用量的既有调度 fallback 继续选路，不能让观测失败阻断推理。
