---
status: superseded
superseded_by: 0064-run-all-model-vendors-as-wasm-plugins
---

# Vendor 按请求用途拥有推理与模型探测的构造契约

> 本 ADR 记录原生 Vendor interface 的历史决策；[ADR-0064](0064-run-all-model-vendors-as-wasm-plugins.md) 已由统一 Wasm Vendor 组件取代该实现形状。请求构造与模型发现知识仍由 Vendor 拥有，但现在位于 guest，并通过 typed SDK/WIT 与受控 host network 执行。
