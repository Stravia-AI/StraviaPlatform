...
- `STRAVIA_UPDATER_PUBLIC_KEY`：对应公钥文件的完整内容、以 `RW` 开头的 key payload，或公钥文件的 base64 编码；workflow 会规范化为 Minisign 两行格式。公钥不是秘密，但以 secret 注入可让 workflow 在构建前统一检查配置完整性。
