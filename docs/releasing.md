...
- `STRAVIA_UPDATER_PUBLIC_KEY`：对应公钥文件的完整内容、以 `RW` 开头的 key payload，或公钥文件的 base64 编码；workflow 会规范化为 Tauri 所需的 base64 编码 Minisign 公钥文件。公钥不是秘密，但以 secret 注入可让 workflow 在构建前统一检查配置完整性。
