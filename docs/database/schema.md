...
| `vendor_options` | JSONB / TEXT | `'{}'` | Vendor 声明的非密钥行为开关（JSON 对象，按 `option_fields` 声明校验，如 command-code 的 `zdr`）；随 Admin API 正常回显 |
...
Pre-admission decode, protocol, or authentication failures remain outside Principal and Generation Chain. Migration 0045 added the nullable failure diagnostics and caller snapshots below for the failed-requests projection; they are optional, never backfilled, and introduce no new Principal foreign key — missing values stay unknown instead of being reconstructed.
...
Exactly one of `run_id` and `rejection_id` is non-NULL. Large Debug payloads are not stored in relational rows. They are segmented JSONL under `<data-dir>/diagnostics/observation-debug`, tracked by retained-byte statistics without capacity limits. The relational manifest permits startup reconciliation of tombstones and orphan managed directories. SQLite resides at `<data-dir>/db/gateway.db`; reorganizing the local layout does not change table definitions or persisted relative object/Trace identities.
