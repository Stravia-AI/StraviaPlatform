...
A client sends a **virtual model name**. That value is the Route ID and is matched exactly, including letter case. The matching Route can contain one or more Targets and use weighted distribution, priority failover, cooldown, or latency selection. Stravia refreshes its Provider Catalog from revisioned `models.stravia.cn` indexes: lightweight Provider and Canonical Model indexes update atomically, while Provider-scoped inventories load only when needed. Catalog-backed Providers use their scoped inventory; account-level discovery remains the source of callable IDs and only enriches exact matches without adding Catalog-only models.
