...
Native compaction uses compatible OpenAI/Codex Targets: `POST /v1/responses/compact` is a separate HTTP unary operation and returns the complete next window, including retained items and opaque state. Replay that window without pruning or rewriting it. Responses also carries native compaction items, Codex in-band triggers, and supported server-side `context_management` controls. These are hard protocol requirements: unsupported Targets cannot silently discard them. A compact operation is not an empty Generation.

Route **Native automatic compaction** is disabled by default. Set a positive input-token threshold to ask a compatible Target to compact its current rendered window; this is not a cumulative usage budget or a model capacity setting. Explicit client controls take precedence, including explicit empty/null values. Disabling the Route policy does not prohibit client-initiated compaction. Known model limits and output reserve constrain the threshold; unknown limits remain unknown and upstream admission errors are returned.
