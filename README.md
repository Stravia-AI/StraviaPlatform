...
The **Debug records** tab is hidden, rather than disabled, for requests without Debug capture. Turning the current Debug switch off does not hide previously captured requests.

Conversation **Thinking** and **Tool call** markers reveal these recorded contents without enabling Debug. Ordinary events take precedence; previously captured Debug records fill missing details without duplicating them. Rows with details start collapsed; each row independently remembers its expanded state in the current browser and site, storing only the state and identifier, not its contents. Thinking without captured readable content is omitted, and tools without details remain plain named rows without missing-data notices. Full captured canonical and application-protocol records still require Debug and remain available under **Debug records**.
