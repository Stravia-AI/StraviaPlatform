...
Media Understanding reads static JPEG, PNG, and WebP images with `{"path":"https://stravia/artifact/<id>"}`. Both owned image references and public image URLs default to description and readable-text extraction; public images are stored first. Add `#stravia?question=Describe%20the%20image` for a specific question, and `&previous_turn_id=<id>` in the same fragment to continue or branch. Explicit `#stravia?download=1` downloads without model execution. HTML with a question remains Markdown and reports `question_applied=false`. If a parent route has an image-capable Target, Stravia delivers the stored image natively; otherwise a tool-capable parent Model can use the configured hidden visual Model. A failed native-vision route does not fall back to that Model.
...
References use `<!-- stravia-redaction-marker:rm_<32 lowercase hexadecimal characters> -->`, an inline member of the History Marker syntax family with separate restoration semantics. Complete markers are excluded from credential detection and are preserved as atoms in diagnostic business text; actual credential headers and structured credential fields remain masked. Requests containing a live reference for the current API Key receive one short system instruction: “Preserve Stravia redaction markers verbatim when used; Stravia restores their values.”

**Marker format upgrade:** migration 0046 converts stored legacy references to the new format while preserving the identifier, secret, Principal, and retention state. Old `~stravia-secret:…~` references in client or upstream history are no longer restored; those histories are not rewritten. Start a new conversation if it still depends on old references.
...
New interactions retain up to 4,096 characters of credential-redacted user-input text with the request records, without requiring Debug. System instructions, historical messages, and tool results are not included in this preview; tool continuations do not replace the original input. Card previews indicate when older records, messages without text, or requests that stopped before input protection finished have no recorded text. The expanded output preview shows the retained output tail, not a guaranteed full response.
