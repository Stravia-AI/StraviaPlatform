...
test.describe('Interaction Observation canvas', () => {
  ...
  test('hides a zero-output terminal failure from chains and reveals it from the failed request', async ({ page }) => {
    await page.emulateMedia({ reducedMotion: 'reduce' })
    const fixture = await installObservationFixture(page)
    ...
    fixture.addFailure(
      failedRequest('run-flint', 'run', startedAt + 280_000, {
        duration_ms: 1_334,
        client: 'codex',
        error: { source: 'upstream', code: 'protocol_lossy_rejected', message: null, status_code: 422 },
    ...
    await page
      ...
      .click()
    const inspector = page.getByRole('complementary', { name: 'Observation details' })
    await expect(inspector.getByRole('button', { name: 'Open interaction', exact: true })).toBeVisible()
    await inspector.getByRole('button', { name: 'Open interaction', exact: true }).click()
    await expect(page.getByRole('tab', { name: 'Interaction Chains', exact: true })).toHaveAttribute(
      'aria-selected',
      'true',
  ...
  test('groups only consecutive same-name client tools and shows attempt-local output speed', async ({ page }) => {
    ...
    const records = diagnostics.locator('li[data-group="tools"]').first().locator('.stream-row[data-sequence]:visible')
    await expect(records).toHaveCount(0)
