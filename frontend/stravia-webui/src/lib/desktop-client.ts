...
async function invokeDesktop<T>(command: string, args?: Record<string, unknown>): Promise<T> {
  if (!isTauri) throw new Error(m.desktop_client_error_desktop_only())
...
export function setDesktopExternalAccess(enabled: boolean): Promise<DesktopPortState> {
  return invokeDesktop('set_desktop_external_access', { enabled })
}

export function getDesktopClientSettings(): Promise<DesktopClientSettings> {
  return invokeDesktop('get_desktop_client_settings')
}

export function setDesktopLaunchAtLogin(enabled: boolean): Promise<DesktopClientSettings> {
  return invokeDesktop('set_desktop_launch_at_login', { enabled })
}

export function setDesktopSilentStart(enabled: boolean): Promise<DesktopClientSettings> {
  return invokeDesktop('set_desktop_silent_start', { enabled })
}
