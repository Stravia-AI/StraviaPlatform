...
import type { CreateProvider, Provider } from '$lib/types'
...
function writeProvider(submission: ProviderConnectionSubmission): Promise<Provider> {
  const { target } = submission
  const credential: CreateProvider['credential'] = oauthProvider
    ? { type: 'none' }
    : Object.keys(submission.credentials).length > 0
      ? { type: 'fields', values: submission.credentials }
      : { type: 'none' }
  const input: CreateProvider = {
    name: submission.name,
    source: {
      type: 'custom',
      vendor: target.descriptor.provider_id,
  ...
  return oauthProvider && submission.sessionId
    ? admin.providers.createOAuth(submission.sessionId, input)
    : admin.providers.create(input)
}

const draft = new ProviderConnectionDraft({
  target: () =>
    selectedOption
      ? { kind: 'create', descriptor: selectedOption.descriptor, channel: selectedOption.channel }
      : undefined,
  api: admin.providers,
  writer: { write: writeProvider },
  oauth: {
...
const oauthSessionSecretFields = $derived(
  draft.oauthReady ? configFields.filter((field) => field.secret && field.required).map((field) => field.key) : [],
)
...
// 提交期间（preview → write → bind）封住应用内导航；刷新/关页由 onbeforeunload 原生提示。
...
$effect(() => {
  const error = draft.submitError
  if (error !== undefined) toast.error(localizeBackendErrorMessage(error))
})

function handleOpenChange(nextOpen: boolean): void {
  open = nextOpen
  if (!nextOpen) void draft.resetConnection()
}

async function chooseOption(option: ProviderOption): Promise<void> {
  const name = providerNameAfterOptionChange(draft.fields.name, selectedOption, option)
  ...
  await draft.resetConnection(fields)
  step = 'configure'
}

async function goBack(): Promise<void> {
  await draft.resetConnection()
  step = 'select'
}
...
<Sheet.Root bind:open onOpenChange={handleOpenChange}>
  <Sheet.Content
    ...
    closeDisabled={draft.submitting}
    escapeKeydownBehavior={draft.submitting ? 'ignore' : 'close'}
    interactOutsideBehavior={draft.submitting ? 'ignore' : 'close'}>
    <Sheet.Header class="border-b">
      ...
      <Tabs.Root value={step} onValueChange={changeStep} class="mt-3">
        <Tabs.List aria-label={m.provider_editor_connection_setup_steps()}>
          <Tabs.Trigger value="select" disabled={draft.submitting}>{m.provider_editor_choose_service()}</Tabs.Trigger>
          <Tabs.Trigger value="configure" disabled={!selectedOption || draft.submitting}
    ...
    {:else if selectedOption}
      <form
        ...
        <div class="route-overlay-body">
          <Field.Group class="grid gap-6 sm:grid-cols-2">
            ...
            <Field.Field size="fill" class="sm:col-span-2">
              ...
              <Input
                ...
                disabled={draft.submitting}
                oninput={() => draft.configurationChanged()} />
            </Field.Field>
            {#if selectedOption.channel.protocols && selectedOption.channel.protocols.length > 1}
              {@const selectedProtocol = selectedOption.channel.protocols.find(
                (option) => option.value === draft.fields.protocol,
              )}
              <Field.Field size="fill" class="sm:col-span-2">
                <Field.Label for="provider-protocol">{m.common_protocol()}</Field.Label>
                <Select.Root
                  type="single"
                  value={draft.fields.protocol}
                  disabled={draft.submitting}
                  onValueChange={(value: string) => {
                    draft.fields.protocol = value
                    draft.configurationChanged()
                  }}>
                  <Select.Trigger id="provider-protocol" class="w-full">
                    {selectedProtocol
                      ? resolvePluginText(selectedProtocol.label, localeState.current)
                      : draft.fields.protocol}
                  </Select.Trigger>
                  <Select.Content>
                    <Select.Group>
            ...
            <ProviderConfigFields
              ...
              disabled={draft.submitting}
              onChanged={() => draft.configurationChanged()} />
          </Field.Group>
          ...
          {#if previewIssueError}
            <p class="mt-6 text-sm text-destructive">{previewIssueError}</p>
          {/if}
        </div>

        <Sheet.Footer class="route-overlay-footer flex-row justify-between sm:justify-between">
          <Button type="button" variant="outline" disabled={draft.submitting} onclick={() => void goBack()}
            >{m.provider_editor_back()}</Button>
          <div class="flex items-center gap-2">
            <Sheet.Close type="button" class={buttonVariants({ variant: 'outline' })} disabled={draft.submitting}>
              {m.common_cancel()}
            </Sheet.Close>
