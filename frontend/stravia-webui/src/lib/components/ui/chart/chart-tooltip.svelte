...
let {
  ...
  formatter?: Snippet<
    [{ value: unknown; name: string; item: TooltipPayload; index: number; payload: TooltipPayload[] }]
  >
} = $props()
...
<TooltipPrimitive.Root variant="none">
  <div
    bind:this={ref}
    class={cn(
      'grid min-w-[9rem] items-start gap-1.5 rounded-lg border border-border/50 bg-background px-2.5 py-1.5 text-xs shadow-lg',
      className,
    )}
    {...restProps}>
    {#if !nestLabel}
      ...
      {#each visibleSeries as item, i (item.key + i)}
        ...
        <div
          ...
          {#if formatter && item.value !== undefined && item.label}
            {@render formatter({ value: item.value, name: item.label, item, index: i, payload: visibleSeries })}
          {:else}
            {#if itemConfig?.icon}
              <itemConfig.icon />
