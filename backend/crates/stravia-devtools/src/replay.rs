//! `stravia-tools replay` — persistent stub upstream that serves recorded
...
pub async fn run(args: ReplayArgs) -> Result<()> {
    ...
    info!(%addr, protocol = %args.protocol, "stravia-tools replay listening");
...
fn build_router(protocol: ProtocolKind, state: Arc<ReplayState>) -> Router {
    ...
    router = match protocol {
        ...
        ProtocolKind::OpenResponses => router.route("/v1/responses", post(handle_body_model)),
...
mod tests {
    ...
    async fn end_to_end_openai_chat_byte_replay() {
        ...
        let dir = tmp.path().join("openai-chat/deepseek");
        std::fs::create_dir_all(&dir).unwrap();
        let body = b"data: {\"choices\":[{\"delta\":{\"content\":\"STRAVIA_PROBE_BASIC_STREAM\"}}]}\n\ndata: [DONE]\n\n";
    ...
    async fn end_to_end_google_content_path_model() {
        ...
        let body = b"data: {\"candidates\":[{\"content\":{\"parts\":[{\"text\":\"STRAVIA_PROBE_BASIC_STREAM\"}]}}]}\n\n";
