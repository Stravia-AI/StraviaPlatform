...
#[tokio::test]
async fn vendor_network_diagnostics_preserve_raw_selected_protocol_ndjson_bytes()
-> anyhow::Result<()> {
    let directory = tempfile::tempdir()?;
    let gateway = new_gateway(directory.path().to_owned()).await?;
    install(&gateway, "lifecycle-v1.wasm", false).await?;
    ...
    assert!(
        !chunks.is_empty(),
        "actual Wasm HTTP response was not captured"
    );
    assert!(chunks.iter().all(|record| {
        record["layer"] == "wire"
    ...
    let captured = chunks.iter().try_fold(Vec::new(), |mut body, record| {
...
#[tokio::test]
async fn vendor_network_wire_only_masks_authorization_while_observations_stay_safe()
-> anyhow::Result<()> {
    ...
    assert!(
        ...
        "raw Wire body/header: {persisted}"
    );
    assert!(
    ...
    let request = records
        .iter()
        .find(|record| {
            record["direction"] == "upstream_request"
                && record["message_type"] == "request"
                && record["url"]
                    .as_str()
                    .is_some_and(|url| url.ends_with("/infer"))
        })
        .expect("actual Wasm HTTP request wire record");
    assert_eq!(request["headers"]["authorization"], "***");
    assert_eq!(request["headers"]["x-license"], LICENSE);
    assert_eq!(request["headers"]["x-plugin-state"], PRIVATE_STATE);
    let request_body = wire_payload_bytes(request)?;
    assert!(
        request_body
            .windows(LICENSE.len())
    ...
    assert!(
        request_body
            ...
            .any(|window| window == PRIVATE_STATE.as_bytes())
    );
    Ok(())
}
