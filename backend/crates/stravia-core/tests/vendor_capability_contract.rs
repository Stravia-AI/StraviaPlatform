...
async fn pure_image_vendor_stores_real_owned_png_and_cannot_chat() -> anyhow::Result<()> {
    ...
    let reference = output["artifact_reference"]
        .as_str()
...
-> anyhow::Result<()> {
    ...
    assert!(
        image
            .structured_content
            .as_ref()
            .is_some_and(|value| value.get("artifact_reference").is_some())
    );
...
-> anyhow::Result<()> {
    ...
    assert!(
        image
            .structured_content
            .as_ref()
            .is_none_or(|value| value.get("artifact_reference").is_none())
    );
    assert_eq!(
        upstream.call_count("/search").await,
        1,
...
async fn bundled_codex_component_reuses_one_connection_for_search_and_image() -> anyhow::Result<()>
{
    ...
    let reference = image["artifact_reference"]
        .as_str()
