...
impl CatalogSource for FailingCatalogSource {
    async fn fetch_version(&self) -> anyhow::Result<CatalogVersion> {
        anyhow::bail!("catalog is unreachable")
    }
    ...
    async fn fetch_provider_scope(&self, provider_id: &str) -> anyhow::Result<Vec<u8>> {
        anyhow::bail!("catalog scope {provider_id} is unreachable")
    }
...
async fn catalog_marker_on_a_live_channel_never_reaches_the_catalog() -> anyhow::Result<()> {
    ...
    .await?;
    gw.provider_catalog =
        ProviderCatalog::with_source(&gw.config.data_dir, Arc::new(FailingCatalogSource))?;
    let provider = gw
        .storage
        .providers()
        .create(CreateProviderRecord {
