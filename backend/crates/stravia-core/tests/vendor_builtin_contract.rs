...
-> anyhow::Result<()> {
    ...
    let generated_records = records
        ...
        .map(|record| {
            serde_json::from_str::<Value>(
                record["payload"].as_str().expect("diagnostic NDJSON text"),
            )
        })
    ...
    assert_eq!(
        generated_records,
        vec![
            json!({"type":"reasoning-delta","text":"because"}),
            json!({"type":"text-delta","text":"done"}),
            json!({"type":"finish","finishReason":"stop","totalUsage":{"inputTokens":7,"outputTokens":3}})
        ],
        "the actual builtin protocol alias must retain every diagnostic NDJSON record"
    );
    for path in ["/alpha/fingerprint/record", "/alpha/lifecycle-events"] {
        let response = records
            ...
            .expect("initialization JSON response in diagnostic bundle");
        let payload: Value =
            serde_json::from_str(response["payload"].as_str().expect("diagnostic JSON text"))?;
