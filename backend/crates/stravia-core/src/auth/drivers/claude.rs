...
impl AuthDriver for ClaudeOAuthDriver {
    ...
    async fn exchange(
        ...
        let callback = parse_oauth_callback(&input, &state.state, "claude")?;
        let code = callback
            .code
            .as_deref()
            .map(str::trim)
    ...
    fn bind_runtime(
        ...
        // The OAuth Bearer cannot call Anthropic's `/v1/models` endpoint.
        // Preserve the channel's curated subscription inventory instead of
        // treating a shared catalog as an account entitlement.
...
mod tests {
    ...
    fn config_loads_from_vendor_registry() {
        ...
        assert_eq!(config.oauth.auth_base_url, "https://claude.com");
        assert_eq!(
            config.oauth.authorize_url,
        ...
        assert_eq!(
            ClaudeOAuthDriver::claude_cli_headers()[0],
    ...
    fn bind_runtime_sets_bearer_oauth_headers_and_disables_default_auth() {
        ...
        assert_eq!(binding.models_source_override.as_deref(), None,);
