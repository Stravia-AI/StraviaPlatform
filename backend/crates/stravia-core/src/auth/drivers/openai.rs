...
use crate::auth::types::{
    ...
    CredentialBundle, ExchangeAuthContext, OAuthCallbackPolicy, OAuthCallbackPort,
    RefreshAuthContext, RuntimeBinding, StartAuthContext, StoredCredential,
};
...
impl AuthDriver for OpenAIOAuthDriver {
    ...
    async fn exchange(
        ...
        let callback = parse_oauth_callback(&input, &state.pkce.state, "openai")?;
        let code = callback
            .code
            .as_deref()
            .map(str::trim)
    ...
    fn bind_runtime(
        ...
        let access_token = credential
            ...
            .map(str::trim)
            .filter(|value| !value.is_empty())
            .ok_or_else(|| anyhow!("openai oauth access token is empty in bind_runtime"))?;
        let mut extra_headers = HashMap::new();
        extra_headers.insert(
            "authorization".to_string(),
            format!("Bearer {access_token}"),
        );
        extra_headers.insert("originator".to_string(), CODEX_ORIGINATOR.to_string());
        extra_headers.insert(
        ...
        extra_headers.insert(
            "version".to_string(),
            config.runtime.models_client_version.to_string(),
        );
...
mod tests {
    ...
    #[test]
    fn codex_runtime_binding_includes_first_party_client_identity() {
        ...
        let binding = OpenAIOAuthDriver
            ...
            .expect("Codex runtime binding");

        assert_eq!(
            binding.extra_headers.get("originator").map(String::as_str),
            Some("codex_cli_rs")
        );
        assert_eq!(
