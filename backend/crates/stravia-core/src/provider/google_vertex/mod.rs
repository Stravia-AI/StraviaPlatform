...
//! provider stores service-account JSON in Adapter Credentials, exchanges it
//! for a short-lived Google access token, and sends
...
const VERTEX_CREDENTIAL_FIELDS: &[CredentialFieldDef] = &[
    CredentialFieldDef {
        key: "project",
...
const METADATA: VendorMetadata = VendorMetadata {
    ...
    channels: &[
        ChannelDef {
            ...
            api_key: None,
            models_source: None,
            capabilities_source: CapabilitiesSource::Catalog("google"),
            static_models: &[
                "gemini-2.5-pro",
                "gemini-2.5-flash",
...
impl Vendor for VertexVendor {
    ...
    fn map_error(&self, status: u16, body: Value) -> GatewayError {
        ...
        {
            Some(message) => {
                GatewayError::upstream_status("google-vertex", status, Some(message.to_string()))
            }
            None => openai_map_error("google-vertex", status, body),
        }
    }
}
...
pub fn is_vertex_vendor(provider: &crate::db::models::Provider) -> bool {
    provider
        ...
        .as_deref()
        .map(str::trim)
        .is_some_and(|value| matches!(value, "google-vertex" | "google-vertex-anthropic"))
}
...
mod tests {
    ...
    fn emits_bearer_header_for_resolved_access_token() {
        let provider = crate::db::models::Provider {
            id: "test".into(),
            name: "test".into(),
            vendor: Some("google-vertex".into()),
