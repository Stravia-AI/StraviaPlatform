...
async fn selected_crlf_ranges_share_the_line_budget_without_context_or_phantom_lines() {
    ...
    let first = page(
        &client,
        &format!("{}#stravia?lines=1-150,201-260", source.reference()),
    )
    .await;
    assert_eq!(
        first["content"],
    ...
    let selected = page(&client, &format!("{snapshot}#stravia?lines=-1")).await;
    assert_eq!(selected["content"], "line 260\r\n");
    let counted = page(&client, &format!("{snapshot}#stravia?lines=259%2B2")).await;
    assert_eq!(counted["content"], "line 259\r\nline 260\r\n");
    for range in ["0", "-0", "-+1", "261", "18446744073709551615%2B2"] {
        assert_eq!(
            call(&client, &format!("{snapshot}#stravia?lines={range}"))
                .await
                .is_error,
            Some(true)
        );
...
async fn empty_snapshot_reads_and_downloads_exactly_empty_text() {
    ...
    assert_eq!(
        call(&client, &format!("{reference}#stravia?lines=1"))
            .await
            .is_error,
        Some(true)
    );
    let download = page(&client, &format!("{reference}#stravia?download=1")).await;
...
async fn cursor_validation_and_expiry_cannot_bypass_snapshot_ownership() {
    ...
    for mutation in mutations {
        ...
        assert_eq!(
            call(&client, &format!("{reference}#stravia?cursor={encoded}"))
                .await
                .is_error,
            Some(true)
        );
    ...
    assert_eq!(
        call(
            &client,
            &format!("{}#stravia?cursor={encoded}", source.reference())
        )
        .await
        .is_error,
        Some(true)
...
async fn media_answer_pagination_preserves_full_history_without_another_model_turn() {
    ...
    let first = page(
        &client,
        &format!("https://stravia/artifact/{}", source.as_str()),
    )
    .await;
    let original = format!("{prefix} [artifact:{}]", source.as_str());
...
async fn untrusted_snapshot_envelopes_are_validated_and_source_truncation_survives_the_last_page() {
    ...
    let download = page(
        &client,
        &format!("{}#stravia?download=1", source.reference()),
    )
    .await;
    let response = reqwest::get(download["download_url"].as_str().unwrap())
        .await
