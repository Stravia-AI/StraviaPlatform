...
const DESCRIPTION: &str = "Generate one image from a prompt, optionally using up to five ordered JPEG/PNG/WebP reference images (owned plain sa: references without read options, or public HTTP(S) image URLs; at most 32 MiB and 25 megapixels each). aspect_ratio and resolution are preferences, not exact guarantees; the current Codex backend clamps to 1024x1024, 1024x1536, or 1536x1024. Returns a stable artifact_reference and actual image dimensions. Reuse it as a reference image, or use StraviaRead with ?download=1 to download. Retries may repeat generation and consume quota.";
