...
/// Type-keyed, request-scoped scratch space shared across all phases (Stravia's
...
pub struct RequestContext {
    ...
    pub trace: TraceSink,
    /// Type-keyed request-scoped scratch space (Stravia's `ngx.ctx`), shared
...
struct CancellationBodyStream {
    inner: Pin<Box<dyn Stream<Item = Result<bytes::Bytes, axum::Error>> + Send>>,
    cancellation: CancellationToken,
    finished: bool,
}

impl Stream for CancellationBodyStream {
...
impl Drop for CancellationBodyStream {
    fn drop(&mut self) {
        if !self.finished {
            self.cancellation.cancel();
        }
...
pub async fn inject_context(mut request: Request, next: Next) -> Response {
    ...
    let stream = CancellationBodyStream {
...
mod tests {
    ...
    #[tokio::test]
    async fn cancellation_waiter_wakes_without_polling() {
    ...
    #[test]
    fn dropping_unfinished_response_body_signals_cancellation() {
        let cancellation = CancellationToken::new();
        let stream = CancellationBodyStream {
            inner: Box::pin(axum::body::Body::empty().into_data_stream()),
            cancellation: cancellation.clone(),
            finished: false,
        };
        drop(stream);
    ...
    #[tokio::test]
    async fn consuming_response_body_does_not_signal_cancellation() {
        let cancellation = CancellationToken::new();
        let stream = CancellationBodyStream {
            inner: Box::pin(axum::body::Body::from("ok").into_data_stream()),
            cancellation: cancellation.clone(),
            finished: false,
        };
