...
async fn protected_reasoning_marker_is_emitted_at_item_done_before_answer() {
    let (reasoning_events, answer_events) =
        openai_responses_protected_reasoning_sse_parts("final answer");
    let (upstream_url, _calls, release_answer) =
        serve_gated_sse(reasoning_events, answer_events).await;
    ...
    let response = execute_stream(gateway, "early-protected-reasoning").await;
    ...
    let prefix = tokio::time::timeout(std::time::Duration::from_secs(2), async {
        ...
        loop {
            let chunk = chunks
                .next()
                .await
                .expect("stream ended before thinking Marker")
                .expect("stream chunk");
            prefix.push_str(std::str::from_utf8(&chunk).expect("UTF-8 stream chunk"));
            if prefix.contains(crate::history_marker::HISTORY_MARKER_PREFIX) {
                break prefix;
            }
        }
    })
    .await
    .expect("thinking Marker should be emitted at ItemDone");
    assert!(!prefix.contains("final answer"), "{prefix}");

    release_answer.send(()).expect("release answer events");
    let remaining = to_bytes(axum::body::Body::from_stream(chunks), usize::MAX)
        .await
        .expect("remaining stream body");
    let body = format!("{prefix}{}", String::from_utf8_lossy(&remaining));
    assert_eq!(
    ...
    let marker_position = body
        .find(crate::history_marker::HISTORY_MARKER_PREFIX)
        .expect("thinking Marker position");
    let answer_position = body.find("final answer").expect("answer position");
    assert!(marker_position < answer_position, "{body}");
}
...
async fn platform_execution_starts_at_tool_call_complete_before_model_turn_ends() {
    ...
    let (upstream_url, _calls, release_upstream) =
        serve_gated_sse(tool_event, "data: [DONE]\n\n".into()).await;
    let data_dir = tempfile::tempdir().expect("temp data dir");
    ...
    .await
    .expect("gateway init");
    configure_route(&gateway, "early-platform-tool", &[upstream_url]).await;

    let response = execute_stream(gateway, "early-platform-tool").await;
    let mut chunks = response.into_body().into_data_stream();
    let prefix = tokio::time::timeout(std::time::Duration::from_secs(2), async {
        let mut prefix = String::new();
        loop {
            let chunk = chunks
                .next()
                .await
                .expect("stream ended before Marker")
                .expect("stream chunk");
            prefix.push_str(std::str::from_utf8(&chunk).expect("UTF-8 stream chunk"));
            if prefix.contains(crate::history_marker::HISTORY_MARKER_PREFIX) {
                break prefix;
    ...
    .await
    .expect("Marker should be emitted at ToolCallComplete");
    assert!(!prefix.contains("[DONE]"), "{prefix}");
    ...
    .await
    .expect("Platform execution should start before Model Turn completion");

    release_upstream
        .send(())
        .expect("release upstream completion");
    let _ = to_bytes(axum::body::Body::from_stream(chunks), usize::MAX)
        .await
        .expect("remaining stream body");
}
...
#[tokio::test]
async fn split_platform_tool_name_and_arguments_never_reach_the_client_stream() {
    let platform_round = [
        serde_json::json!({
            "id": "upstream-split-tool",
            "model": "provider-model",
            "choices": [{
                "index": 0,
                "delta": {
    ...
    .collect::<String>()
        + "data: [DONE]\n\n";
    let (upstream_url, calls) =
        serve_sse_sequence(vec![platform_round, openai_sse("final after split tool")]).await;
    let data_dir = tempfile::tempdir().expect("temp data dir");
    let tool_calls = Arc::new(std::sync::Mutex::new(Vec::new()));
    let (hook, _) = ExposeOrderedToolHook::counting();
    let (gateway, _logs) = Gateway::builder(crate::config::GatewayConfig {
    ...
    .await
    .expect("gateway init");
    configure_route(&gateway, "split-platform-tool", &[upstream_url]).await;

    let response = execute_stream(gateway, "split-platform-tool").await;
    let status = response.status();
    let body = to_bytes(response.into_body(), usize::MAX)
        .await
        .expect("split tool stream body");
    let body = String::from_utf8_lossy(&body);
    assert_eq!(status, StatusCode::OK, "{body}");
    assert!(body.contains("stravia-history-marker:"), "{body}");
    assert!(body.contains("final after split tool"), "{body}");
    assert!(!body.contains("stravia__ord"), "{body}");
    assert!(!body.contains("ered_tool"), "{body}");
    assert!(!body.contains("call-split"), "{body}");
