...
impl StreamFailure {
    ...
    pub fn to_outcome_code(&self) -> &'static str {
        match self {
            Self::Upstream { .. } => "STRAVIA_UPSTREAM_ERROR",
            Self::Parse { .. } => "STRAVIA_STREAM_PARSE_ERROR",
            Self::ClientCancelled => "STRAVIA_CLIENT_CANCELLED",
            Self::Timeout => "STRAVIA_UPSTREAM_TIMEOUT",
            Self::Internal { .. } => "STRAVIA_INTERNAL_ERROR",
        }
    }
}
