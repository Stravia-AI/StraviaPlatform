        ...
        Ok(sqlx::query_as::<_, crate::router::TargetSchedulingSnapshot>(
            "SELECT provider_id || ':' || upstream_model AS target_key,
                    COALESCE(SUM(CASE WHEN client_status_code BETWEEN 200 AND 299
                                     ...
                                     THEN input_tokens ELSE 0 END), 0) AS input_tokens_24h,
                    COALESCE(SUM(CASE WHEN client_status_code BETWEEN 200 AND 299
                                          AND upstream_status_code BETWEEN 200 AND 299
                                     THEN output_tokens ELSE 0 END), 0) AS output_tokens_24h,
                    COALESCE(SUM(CASE WHEN client_status_code BETWEEN 200 AND 299
                                          AND upstream_status_code BETWEEN 200 AND 299
                                     THEN cache_read_tokens ELSE 0 END), 0) AS cache_read_tokens_24h,
                    COALESCE(SUM(CASE WHEN client_status_code BETWEEN 200 AND 299
                                          AND upstream_status_code BETWEEN 200 AND 299
                                     THEN cache_write_tokens ELSE 0 END), 0) AS cache_write_tokens_24h,
                    COALESCE(SUM(CASE WHEN created_at >= ? THEN 1 ELSE 0 END), 0) AS attempts_1h,
                    COALESCE(SUM(CASE WHEN created_at >= ?
                                          AND client_status_code BETWEEN 200 AND 299
                                          AND upstream_status_code BETWEEN 200 AND 299
                                     THEN 1 ELSE 0 END), 0) AS successes_1h,
                    COALESCE(SUM(CASE WHEN created_at >= ?
                                          AND client_status_code BETWEEN 200 AND 299
                                          AND upstream_status_code BETWEEN 200 AND 299
                                     THEN output_tokens ELSE 0 END), 0) AS successful_output_tokens_1h,
                    COALESCE(SUM(CASE WHEN created_at >= ?
                                          AND client_status_code BETWEEN 200 AND 299
                                          AND upstream_status_code BETWEEN 200 AND 299
                                     THEN latency_upstream_ms ELSE 0 END), 0) AS successful_upstream_ms_1h,
