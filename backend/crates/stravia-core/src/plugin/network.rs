...
static NEXT_HTTP_CAPTURE_ID: AtomicU64 = AtomicU64::new(1);
...
impl VendorNetwork {
    ...
    pub(crate) fn new(
        ...
        // Diagnostics parse canonical wire identities; custom plugin protocols remain opaque.
    ...
    pub(crate) fn http_start(
        ...
        network.capture_id = network
            .observer
            .as_ref()
            .filter(|observer| observer.debug_enabled())
            .map(|_| NEXT_HTTP_CAPTURE_ID.fetch_add(1, Ordering::Relaxed));
