...
pub fn search_report_schema() -> Value {
    serde_json::json!({
        "type": "object",
...
#[derive(Default)]
pub struct LocalSearchEvidenceStore {
...
pub struct LocalSearchOutputValidator {
...
pub struct LocalSearchBackend {
    runner: Arc<dyn crate::host::LocalSearchHost>,
    evidence_store: Arc<LocalSearchEvidenceStore>,
}
...
impl SearchBackend for LocalSearchBackend {
    ...
    async fn run(&self, input: SearchBackendInput) -> Result<BackendOutput, WebSearchError> {
        ...
        while let Some(event) = stream.next().await {
            match event {
                stravia_runtime_contract::agent::AgentEvent::Completed(result)
                | stravia_runtime_contract::agent::AgentEvent::Partial(result) => {
                ...
                stravia_runtime_contract::agent::AgentEvent::ModelStepStarted { .. } => {
                    model_turns = model_turns.saturating_add(1);
                }
                stravia_runtime_contract::agent::AgentEvent::ToolStarted { .. } => {
...
pub fn message_text(message: &AiItem) -> Option<&str> {
