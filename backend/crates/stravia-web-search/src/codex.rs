...
#[derive(Clone)]
pub struct CodexAgenticSearchBackend {
    gateway: std::sync::Arc<dyn crate::host::CodexHost>,
}

impl CodexAgenticSearchBackend {
    pub fn new(gateway: std::sync::Arc<dyn crate::host::CodexHost>) -> Self {
        Self { gateway }
    }
}

pub fn codex_provider_contract(provider: &crate::host::SearchProvider) -> bool {
    provider.is_enabled
...
impl SearchBackend for CodexAgenticSearchBackend {
    ...
    async fn run(&self, input: SearchBackendInput) -> Result<BackendOutput, WebSearchError> {
        ...
        let session =
            self.gateway.provider(provider_id).await?.ok_or_else(|| {
                codex_error("provider_unavailable", "Codex Provider is unavailable")
            })?;
        if !codex_provider_contract(session.provider()) {
            return Err(codex_error(
                "provider_invalid",
        ...
        let runtime = session.transport().await?;
        ...
        for (name, value) in runtime.extra_headers {
