            ...
            .and_then(|p| p.as_array())
        {
            for part in parts {
                if part.get("text").is_some() {
                    if let Some((text, is_thought, thought_signature)) = canonical_text_part(part) {
                        if is_thought {
                            items
                                .push(AiItem::thinking(text, thought_signature.map(str::to_owned)));
                        } else {
                ...
                if part.get("functionCall").is_some() {
                    let Some((function_call, thought_signature)) =
                        canonical_function_call_part(part)
                    else {
                        items.push(AiItem::unknown(part.clone()));
                        continue;
                    };
                    if let Some(signature) = thought_signature {
                        items.push(AiItem::thinking(String::new(), Some(signature.to_owned())));
                    }
...
fn canonical_text_part(part: &Value) -> Option<(&str, bool, Option<&str>)> {
    ...
    let thought = match fields.get("thought") {
        None | Some(Value::Null) => false,
        Some(Value::Bool(thought)) => *thought,
        Some(_) => return None,
    };
    let thought_signature = match fields.get("thoughtSignature") {
    ...
    Some((
        text,
        thought || thought_signature.is_some(),
        thought_signature,
    ))
}

fn canonical_function_call_part(part: &Value) -> Option<(&Map<String, Value>, Option<&str>)> {
    let fields = part.as_object()?;
    if !fields
        .keys()
    ...
    let function_call = fields.get("functionCall")?.as_object()?;
    if !function_call
        .keys()
        .all(|key| matches!(key.as_str(), "id" | "name" | "args"))
        || !function_call.get("name").is_some_and(Value::is_string)
        || function_call
            .get("id")
    ...
    Some((function_call, thought_signature))
}
