...
impl ProtocolRegistry {
    ...
    pub fn adapter(&self, id: &ProtocolEndpoint) -> Option<&Arc<dyn ProtocolAdapter>> {
        self.by_id.get(id)
    }
    ...
    /// 4. Legacy enum string (e.g. `openai`, `gemini`)
    /// 5. Registered protocol suite, using its first canonical endpoint.
    ///
    ...
    pub fn resolve_alias(&self, raw: &str) -> Option<ProtocolEndpoint> {
        ...
        let protocol = self.protocol_aliases.get(lower.as_str())?;
        self.by_id
            .keys()
    ...
    pub fn protocol_supports_function_calling(&self, raw: &str) -> bool {
    ...
    pub fn protocol_represents_target_thinking_control(
        &self,
        raw: &str,
        control: &stravia_runtime_contract::thinking::TargetThinkingControl,
    ) -> bool {
        Protocol::from_identifier(raw)
...
mod tests {
    ...
    #[test]
    fn registers_only_standard_adapters() {
    ...
    fn alias_table_resolves_old_canonical_and_short() {
        ...
        assert_eq!(
            reg.resolve_alias("openai-compatible"),
            Some(OPENAI_COMPATIBLE_CHAT_COMPLETIONS_V1)
        );
        assert_eq!(
            reg.resolve_alias("google-gemini"),
            Some(GOOGLE_GEMINI_GENERATE_CONTENT_V1BETA)
        );
        assert_eq!(
    ...
    #[test]
    fn list_protocols_returns_all_standard_protocols() {
        let reg = ProtocolRegistry::global();
