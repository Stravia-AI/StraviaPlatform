...
fn parse_response(body: &[u8]) -> Result<AllowanceResponse, ()> {
    ...
    if let Some(status) = message(&user, 13) {
        ...
        // PlanStatus #16 is a signed int64 micro-dollar balance. Protobuf
        // exposes its two's-complement varint as u64, so reinterpret it before
        // conversion or an overdrawn account becomes a phantom ~1.8e13 USD.
        if let Some(balance_micro) = varint(&status, 16) {
            let mut allowance = item("balance_usd", "Balance", "balance");
            allowance.remaining = Some(amount(
...
mod tests {
    ...
    use crate::codec::devin_connect::proto::{write_message_field, write_varint_field};

    #[test]
    fn reports_negative_overage_balance() {
        // PlanStatus #16 is int64 -730000 micro-dollars (-$0.73). Treating
        // the decoded varint as unsigned produced a huge phantom balance.
        ...
        let parsed = parse_response(&body).expect("Devin user status should parse");
        let balance = parsed
            .allowances
            .iter()
