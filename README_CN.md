...
Codex OAuth 优先使用 Responses 中携带 `compaction_trigger` 的 V2 远程压缩。通过 `response.output_item.done` 已交付的原生状态，在终态响应未重复该条目时仍然有效；显式重复的状态必须保持一致。独立 compact 端点是否可用由上游决定：上游返回 404 时仍返回错误，不伪造成功，也不隐式切换成 V2。
...
WebSocket 客户端在 Interaction 等待工具结果期间断开连接时，请求记录会把该连接所属、尚未续接的等待叶分支切换为**已断开**。已完整交付的响应与 Generation Chain 仍可用于后续续接。HTTP/SSE 响应正常结束不能证明客户端离线；无法确定连接归属的旧记录不回填断线状态。

等待分支的全部客户端工具调用在同一 Interaction 内收到无歧义的后续结果后，即使结果来自 sibling Run，该分支也不再阻塞交互完成。重启时，请求记录会纠正这些已回传等待的聚合状态，并把上一进程尚未回传的等待叶分支标为**已中断**（`process_restarted`）；不删除历史，也不改写已交付响应。之后仍可合法续接。
...
概览根据已加载的配置推荐一个下一步操作，不以请求记录判断接入是否完成。连接模型服务后，可搜索清单并沿用上游 ID 添加所需模型；Model ID 精确相同时，会把该服务加入已有模型而非重复创建。添加动作始终可见；成功后用 toast 提示，清单保持打开，可继续添加其他模型。接入客户端仍走导航和概览入口。已配置与已启用数量只描述保存的设置，不代表上游连接已经验证成功。
...
- 支持全屏的实时 Interaction 因果森林请求记录、覆盖准入前拒绝与最终失败 Run 的失败的请求列表、时间序检查器及 Confirmed Upstream Usage 统计；实时预设按 5、10、30 分钟及 1、4、12、24 小时滚动，也可精确选择本地日期时间范围，边界固定且最长 24 小时。交互链路按根链最新活动选中整棵根链并保留完整因果上下文，失败的请求按每次请求的开始时间筛选；工具栏可进入或退出全屏，Esc 可退出全屏
...
记录中的思考使用 Markdown 空行分隔可识别的 part，以及结束后再次开始的思考段，实时显示与历史回放保持一致。同一 part 内的增量保持连续，展示空白不改变 canonical 思考原文。已经丢失 part 边界的旧记录不会改写。
...
检查器首次加载最新 200 条事件，之后增量读取；向上滚动接近顶部时自动分批加载更早记录，并保持阅读位置。实时文本先以未保存状态显示，提交后替换预览，不重复文本，也不拆开 Markdown 消息。新文本块达到 16 KiB 或约两秒时封口，只在体积下降时压缩。正常调度下允许约两秒未落盘窗口，进程崩溃可能丢失这部分观察文本；存储失败单独提示，该窗口不是故障期间的持久化时限保证。普通详情读取不强制落盘，下载诊断包只刷新选中的 Interaction，并包含固定截止点内的完整历史。既有事件不改写、不删除。
...
Observation 元数据与托管 Debug Trace segment 共用 `log_retention_days`（默认七天）。Debug Trace 不设容量上限，仅统计已保留字节；队列、writer 或存储造成的丢失会显示为明确 gap 或 partial Trace，绝不改变推理结果。Debug 开启时提供「清除 Debug 数据」，删除全部已保留 Trace，不影响请求记录与开关状态。清除历史会保留 running 与 waiting-client Interaction，并报告跳过数量。Interaction 或 Rejected Request 可通过 60 秒、单次使用的下载 ticket 导出时间点固定的流式 ZIP；manifest 会记录 event sequence 边界和 `complete`、`partial` 或 `none` 状态。实时更新、Debug 状态、Trace 存储及 ticket 均限于单个 Gateway 进程；不提供集群级 fanout、共享 capture 存储或跨实例 ticket。
