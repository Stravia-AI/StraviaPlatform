...
未启用 Debug 采集的请求直接隐藏 **Debug 记录**标签，不显示灰色禁用项。关闭当前 Debug 开关不影响查看之前已经采集的请求。

对话中的**思考**与**调用工具** Marker 可展开这些已记录内容，不自动开启 Debug。普通事件优先，已有 Debug 记录仅补充缺失详情，不重复展示。有详情的条目默认折叠，每条独立在当前浏览器、当前站点记住展开状态，只保存状态与标识，不保存正文。没有可读思考正文时不显示思考条目；工具没有详情时只显示名称，不添加缺失说明。完整 canonical 与应用协议记录仍需开启 Debug，并在 **Debug 记录**中检查。
...
输出浮层命名为**模型输出预览**；已确认执行来源的连线保留，但不再重复显示文字标签。
