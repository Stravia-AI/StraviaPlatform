...
- Debug capture decodes declared-base64 fragments before UTF-8/JSON reassembly, recognizes existing structured Artifact references, and does not treat pending null media fields as lost content. Failed file writes publish only confirmed complete-record boundaries. Client tool-result deduplication avoids copying large batch payloads while retaining its existing lineage, handoff, and uncertainty boundaries.
- Observation now coalesces adjacent same-scope text before enqueueing, preventing fine-grained streaming bursts from displacing request admission and completion records. Debug capture scans fragmented JSON incrementally and batches small file writes; strict validation, redaction, ordered export boundaries, and explicit partial-capture errors remain intact. Existing missing observations and split interaction links are not reconstructed.
...
- Debug traces retain the per-Model Turn `canonical_request` checkpoint alongside the Artifact-normalized request, preserving the documented diagnostic stage sequence.
