...
- Command Code Debug capture reassembles NDJSON records across network chunks and split UTF-8 bytes, preserves complete EOF records, and marks malformed or incomplete tails as partial without discarding earlier complete records. Plugin HTTP captures keep concurrent responses isolated even at the same URL, so initialization and auxiliary calls cannot consume inference records.
