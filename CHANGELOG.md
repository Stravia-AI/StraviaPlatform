...
- OpenAI-compatible streams now deliver ordinary Text immediately even when Platform Tools are exposed. Thinking after the first Text is projected as a Markdown blockquote in `content` with an authoritative History Marker, preserving replay order without Model Leg-sized buffering.
